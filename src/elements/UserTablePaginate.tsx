...
const UserTablePaginate = (props: {
  user: Array<User>; getUserApi: Function;
  ...
  const handleDelete = async (id: any) => {
    ...
    try {
      const { data, status } = await instance.delete(`${ApiRouteList.USER}/${id}?role=${ROLE}`);
      if (status === 200) {
    ...
    <>
      <div style={{
        ...
        <Table celled style={{ minWidth: "300px" }}>
          ...
          <Table.Body>
            {props.user.map((info: User, index: number) => (
              <Table.Row key={index}>
                <Table.Cell>{info.firstName}</Table.Cell>
                <Table.Cell>{info.lastName}</Table.Cell>
                <Table.Cell>{info.email}</Table.Cell>
                <Table.Cell>
                  <AddEditUserModal changePage={props.changePage} current={props.current} userInfo={info} /> 
                  <BsFillTrashFill style={{cursor : 'pointer', color : 'red', marginLeft: '15px'}} onClick={() => handleDelete(info._id)} />
                </Table.Cell>
              </Table.Row>
            ))}
          </Table.Body>
