...
import { LessonType } from "../constants/enum";
...
const LessonTablePaginate = (props: {
  lesson: Array<LessonType>; getLessonApi: Function;
  ...
  const handleDelete = async (id: any) => {
    ...
    try {
      const { data, status } = await instance.delete(`${ApiRouteList.TRAINING_CONTENT}/${id}?role=${ROLE}`);
      if (status === 200) {
        ...
        <Table celled style={{ minWidth: "300px" }}>
          <Table.Header>
            <Table.Row>
              ...
              <Table.HeaderCell width={3}>Resource URL</Table.HeaderCell>
              <Table.HeaderCell width={1}>Feedback URL</Table.HeaderCell>
              <Table.HeaderCell width={2}>Actions</Table.HeaderCell>
            </Table.Row>
          </Table.Header>
          <Table.Body>
            {props.lesson.map((info: LessonType, index: number) => (
              <Table.Row key={index}>
                <Table.Cell>{info.title}</Table.Cell>
                <Table.Cell>{info.description}</Table.Cell>
                <Table.Cell>{info.resourceURL}</Table.Cell>
                <Table.Cell>{info.tag}</Table.Cell>
                <Table.Cell>
                  <AiFillEye
                    type="button"
...
export default LessonTablePaginate;
