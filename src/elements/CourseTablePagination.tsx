...
const CourseTablePaginate = (props: {
  course: Array<CourseType>; getCourseApi: Function;
  ...
  const handleDelete = async (id: any) => {
    ...
    try {
      const { data, status } = await instance.delete(`${ApiRouteList.COURSE}/${id}?role=${ROLE}`);
      if (status === 200) {
  ...
  return (
    <>
      <div style={{
        ...
        <Table celled style={{ minWidth: "300px" }}>
          <Table.Header>
            <Table.Row>
              ...
              <Table.HeaderCell>Course Image URL</Table.HeaderCell>
              <Table.HeaderCell>Actions</Table.HeaderCell>
            </Table.Row>
          </Table.Header>

          <Table.Body>
            {props.course.map((info: CourseType, index: number) => (
              <Table.Row key={index}>
                <Table.Cell>{info.title}</Table.Cell>
                <Table.Cell>{info.description}</Table.Cell>
                <Table.Cell>{info.courseURL}</Table.Cell>
                <Table.Cell>
                  <AddEditCourseModal changePage={props.changePage} current={props.current} userInfo={info} />  <BsFillTrashFill style={{cursor : 'pointer', color : 'red', marginLeft: '15px'}} onClick={() => handleDelete(info._id)} />
                </Table.Cell>
              </Table.Row>
            ))}
          </Table.Body>
        ...
        {props.course.length == 0 && <div style={{
...
export default CourseTablePaginate;
